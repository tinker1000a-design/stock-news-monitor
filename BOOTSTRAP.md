...
prompt 必须自包含：先读本仓库 BOOTSTRAP.md 第 6-7 节流程；标的与别名列表；vault 路径 `<config.vault.path>`；拉取当日新闻/公告/资金流→三维筛选→按日报模板写 `<vault>/日报/YYYY-MM-DD-<标的>-<核心命题>.md`→登记可证伪命题→更新个股中枢时间线→回写 state.json。数据源按第 1 节降级链，不可用时写"数据源不可用"日报，禁止凭记忆补数据。

**任务 2 · 周五复盘周报** `FREQ=WEEKLY;BYDAY=FR;BYHOUR=15;BYMINUTE=30`
prompt 必须自包含：扫描 `<vault>/命题登记/` 下 `结果: 待观察` 且 `验证截止 <= 今天` 的命题**逐条打分**（成立/证伪/待观察，一个不许跳）；汇总本周日报；盲区引擎三项（覆盖矩阵/反向信息注入/证据漏斗）；按周报模板写 `<vault>/周报/YYYY-WW-周报.md`；证伪信息回写图谱（连续证伪 3 次的信源降权）。
